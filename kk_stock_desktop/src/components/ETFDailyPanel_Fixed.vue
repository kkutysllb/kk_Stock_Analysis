<template>
  <div class="etf-daily-panel-fixed">
...
const loadETFList = async () => {
  if (isDestroyed || loadingETFList.value) return
...
const aiDataContext = computed(() => {
  ...
  if (chartData.length > 0) {
    ...
    const isDataMatching = !hasValidTsCode || 
                          ...
                          chartData.some(item => item.ts_code === selectedETF.value)
    ...
    if (isDataMatching) {
      ...
      console.warn(`chartData数据与当前选中ETF(${selectedETF.value})不匹配，使用latestData数据`)
      validChartData = []
  ...
  let summary = ''
  if (validLatestData) {
    const pctChg = validLatestData.pct_chg || 0
...
const loadETFData = async () => {
  // 检查组件状态
  if (!selectedETF.value || isDestroyed) {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      if (response.data.daily_data && Array.isArray(response.data.daily_data)) {
        chartData = response.data.daily_data
          .sort((a: any, b: any) => {
...
onMounted(async () => {
  ...
  try {
    ...
    if (!isDestroyed) {
      ...
      await nextTick()
      if (!isDestroyed && etfList.value.length > 0) {
        loadETFData()
      }
    }
