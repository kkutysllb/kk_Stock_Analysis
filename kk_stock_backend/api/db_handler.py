...
# 数据库配置
CLOUD_MONGO_URI = "mongodb://root:example@118.195.242.207:27017/quant_analysis?authSource=admin"
...
_db_handler_lock = None

def get_db_handler():
    """获取数据库处理器单例实例"""
    global _db_handler_instance, _db_handler_lock
    
    if _db_handler_lock is None:
        import threading
        _db_handler_lock = threading.Lock()
...
class DBHandler:
    ...
    def _connect_databases(self):
        ...
        if self.local_available:
            ...
            print(f"❌ Worker{worker_id}: 本地数据库连接失败，服务无法启动")
            raise Exception("本地数据库连接失败，请检查MongoDB服务状态")
