<template>
  <div class="swan-indices-panel" v-if="isMounted">
    <!-- 指数选择器 -->
...
const props = withDefaults(defineProps<Props>(), {
  initialIndex: '801010.SI',  // 默认为列表第一项：农林牧渔
  initialPeriod: 'daily'
})
// 定义类型
...
const indexLatestData = shallowRef<any>(null)
...
onUnmounted(() => {
  ...
  try {
    ...
    chartData.value = []
    indexLatestData.value = null
  } catch (error) {
    // 完全忽略卸载时的任何错误
    console.warn('组件卸载时出现错误，已忽略:', error)
  }
})

// 计算属性：获取当前选中指数的名称
const selectedIndexName = computed(() => {
  const index = swanIndices.value.find(item => item.code === selectedIndex.value)
  return index ? index.name : '请选择申万行业指数'
})
...
const onIndexChange = (value: string) => {
  emit('update:selected-index', value)
  loadIndexData()
}

const onPeriodChange = async (value: PeriodType) => {
...
const loadIndexData = async () => {
  // 检查组件状态
  if (!selectedIndex.value || componentUnmounted.value || !isMounted.value) {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      if (response.data.sw_industry_data && Array.isArray(response.data.sw_industry_data)) {
        chartData.value = response.data.sw_industry_data
          .sort((a: any, b: any) => {
        ...
        if (chartData.value.length > 0) {
          indexLatestData.value = chartData.value[chartData.value.length - 1]
        }
...
const initChart = () => {
  ...
  try {
    ...
    // 存储清理函数，在组件卸载时统一调用
    const addCleanupFunction = (cleanupFn: () => void) => {
...
// 组件卸载时清理（由于已经在上面的onUnmounted中处理，这里删除重复的钩子）
...
const aiDataContext = computed(() => {
  ...
  if (chartData.value.length > 0) {
    ...
    if (isDataMatching) {
      ...
      // 数据不匹配，使用indexLatestData并清空chartData
      console.warn(`chartData数据与当前选中指数(${selectedIndex.value})不匹配，使用indexLatestData数据`)
      validChartData = []
      latestData = indexLatestData.value
    }
  } else {
    // 如果chartData为空，则使用indexLatestData
    validChartData = []
    latestData = indexLatestData.value
  }
  ...
  let summary = ''
  if (latestData) {
    const pctChg = latestData.pct_change || 0
...
- 行业特征：${currentIndex?.name}作为申万行业指数，反映该行业整体走势和投资热度
- 技术指标：请结合价格走势、成交量变化、涨跌幅分布等进行综合分析

请基于以上完整的${currentIndex?.name}数据进行深入分析，提供该行业的投资建议、风险提示和市场前景判断。
...
/* 行业名称样式 */
:deep(.el-select-dropdown__item .index-name) {
  font-weight: 500 !important;
  flex: 1 !important;
}

/* 代码样式 */
:deep(.el-select-dropdown__item .index-code) {
  color: #888 !important;
