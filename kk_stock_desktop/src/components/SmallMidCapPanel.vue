<template>
    <div class="component-indices-panel">
      ...
      <div class="panel-header">
        <div class="panel-title-section">
          <h3 class="panel-title">中小板块指数分析</h3>
          <AskAIComponent :data-context="aiDataContext" />
        </div>
        <div class="index-selector">
          <el-radio-group v-model="selectedIndex" @change="onIndexChange" size="default" class="index-selector">
            <el-radio-button 
            ...
            >
              <span class="index-content">
                ...
                <span v-if="getIndexLatestData(index.code)" class="index-price">
                  ...
                  >
                    {{ (getIndexLatestData(index.code)?.pct_chg || 0) > 0 ? '+' : '' }}{{ getIndexLatestData(index.code)?.pct_chg?.toFixed(2) || '0.00' }}%
                  </span>
                </span>
                <span v-else class="index-price">
                  <span v-if="index.code.endsWith('.CSI') && (activePeriod === 'weekly' || activePeriod === 'monthly')" 
                        class="price-change text-gray-500">
                    无{{ activePeriod === 'weekly' ? '周线' : '月线' }}数据
                  </span>
                  <span v-else class="price-change">加载中...</span>
                </span>
              </span>
            </el-radio-button>
          </el-radio-group>
  ...
  initialIndex: '000510.CSI',  // 默认为中证A500
  initialPeriod: 'daily'
})
  ...
  // 中小板块指数配置
const indexConfig = [
  { code: '000510.CSI', name: '中证A500', color: '#3b82f6' },
  { code: '000142.SH', name: '上证380', color: '#10b981' },
  { code: '399010.SZ', name: '深证700', color: '#f59e0b' },
  { code: '399020.SZ', name: '创业小盘', color: '#ef4444' },
  { code: '932000.CSI', name: '中证2000', color: '#8b5cf6' }
]
...
const loadAllIndexData = async () => {
  // 防止重复加载
  if (loading.value) {
    // console.log('SmallMidCapPanel: 数据正在加载中，跳过重复请求')
    return
  }
  ...
  try {
    // console.log('SmallMidCapPanel: 开始加载指数数据...') 
    // 获取指数数据
    ...
    // console.log('SmallMidCapPanel: API响应:', response)
    
    if (response.success && response.data && response.data.indices) {
      // console.log(`SmallMidCapPanel: 收到${response.data.indices.length}个指数数据`)
      ...
      response.data.indices.forEach(index => {
        if (index && index.latest_data) {
          ...
          // 如果是周或月粒度，且pct_chg为空或0，尝试计算环比涨跌幅
          if ((activePeriod.value === 'weekly' || activePeriod.value === 'monthly') && 
              (!latestData.pct_chg || latestData.pct_chg === 0 || isNaN(latestData.pct_chg))) {
            if (latestData.close && latestData.pre_close && latestData.pre_close !== 0) {
              ...
              // console.log(`SmallMidCapPanel: 手动计算${index.ts_code}的涨跌幅: ${latestData.pct_chg.toFixed(2)}%`)
            }
          }
          ...
          // console.log(`SmallMidCapPanel: 处理指数 ${index.ts_code} (${index.name}):`, latestData)
        }
      })
      
      // console.log('SmallMidCapPanel: 最终indexDataMap:', indexDataMap.value)
      
      // 检查中小板块指数是否都有数据
      indexConfig.forEach(config => {
        if (!indexDataMap.value[config.code]) {
          console.warn(`SmallMidCapPanel: 缺少指数数据 ${config.code} (${config.name})`)
        }
      })
    } else {
      console.error('SmallMidCapPanel: API响应无效:', response)
    }
  } catch (error) {
    console.error('SmallMidCapPanel: 加载指数数据失败:', error)
  } finally {
    loading.value = false
  }
}
...
// 调试函数：检查指数数据获取情况
const debugIndexData = () => {
  // console.log('=== SmallMidCapPanel 调试信息 ===')
  // console.log('indexDataMap:', indexDataMap.value)
  
  indexConfig.forEach(config => {
    const data = getIndexLatestData(config.code)
    // console.log(`${config.name} (${config.code}):`, data)
    if (data) {
      // console.log(`  - 收盘价: ${data.close}`)
      // console.log(`  - 涨跌幅: ${data.pct_chg}%`)
      // console.log(`  - 交易日期: ${data.trade_date}`)
    } else {
      // console.log('  - 无数据')
      
      // 检查是否是CSI指数且周期为weekly/monthly
      if (config.code.endsWith('.CSI') && (activePeriod.value === 'weekly' || activePeriod.value === 'monthly')) {
        // console.log(`  - 注意: ${config.name} 在Tushare中没有${activePeriod.value === 'weekly' ? '周线' : '月线'}数据`)  
      }
    }
  })
  
  // 检查当前周期
  // console.log(`当前周期: ${activePeriod.value}`)
  // console.log(`数据点数: ${dataPoints.value}`)
}
  ...
  - 数据范围：${activePeriod.value === 'custom' && dateRange.value ? `${dateRange.value[0]} 至 ${dateRange.value[1]}` : `最近${dataPoints.value}个交易日`}
     - 指数特征：${currentIndexConfig?.name}作为中小板块指数，反映中小市值股票的整体表现
  ...
  【再次提醒：本次分析的重点对象是 ${currentIndexConfig?.name}，代码 ${selectedIndex.value}，请勿混淆其他指数】`
   ...
   请等待数据加载完成后进行分析。`
         ...
         return {
       type: 'small_mid_cap_index',
       name: `${currentIndexConfig?.name || '中小板块指数'}(${selectedIndex.value})`, // 这个name字段用于AI组件检测数据变化
       title: '中小板块指数分析',
  ...
  // 监听周期变化时重新加载所有数据
  watch([activePeriod, dataPoints], () => {
    loadAllIndexData()
  })
  
  // 事件处理函数
  const onIndexChange = (value: string | number | boolean | undefined) => {
    const stringValue = String(value)
    emit('update:selected-index', stringValue)
    loadIndexData()
  }
  
  const onPeriodChange = (value: string | number | boolean | undefined) => {
  ...
  onMounted(async () => {
    try {
      ...
      console.error('SmallMidCapPanel: 组件初始化失败:', error)
    }
