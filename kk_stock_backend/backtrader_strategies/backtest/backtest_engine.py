...
class BacktestEngine:
    ...
    def load_data(self, stock_codes: Optional[List[str]] = None, max_stocks: int = 50):
        ...
        if stock_codes is None:
            # 检查策略是否有指定股票池
            if hasattr(self.strategy, 'get_stock_universe'):
                stock_codes = self.strategy.get_stock_universe()
                self.logger.info("策略指定了自定义股票池")
            else:
                # 默认使用指数股票池
        ...
        # 通用市场数据加载逻辑 - 使用策略适配器选股
        try:
            # 检查策略是否有策略适配器
            strategy_type = getattr(self.strategy, 'strategy_type', '')
            if hasattr(self.strategy, 'get_strategy_adapter'):
                # 策略提供了策略适配器
                adapter = self.strategy.get_strategy_adapter()
                self.logger.info(f"🎯 使用策略适配器进行选股: {strategy_type}")
                
                import asyncio
                
                # 使用策略适配器选股
                screening_result = asyncio.run(adapter.screen_stocks(
                    stock_pool="all",  # 全市场选股
                ...
                self.logger.info(f"🎯 策略适配器选出{len(selected_stock_codes)}只候选股票")
                
                if len(selected_stock_codes) == 0:
                    raise ValueError("策略适配器选股失败，未找到符合条件的股票")
                ...
                self.market_data = self.data_manager.load_market_data(
                    ...
                    strategy_scorer=None  # 策略适配器有自己的选股逻辑
                )
                
                self.logger.info(f"📈 为{strategy_type}加载了 {len(self.market_data)} 只股票的市场数据")
            ...
            import traceback
            traceback.print_exc()
            raise ValueError(f"数据加载失败: {e}") from e
